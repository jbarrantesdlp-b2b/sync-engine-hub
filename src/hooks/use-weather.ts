...
const CACHE_KEY = "sync-weather-v2";
...
export function useWeather() {
  ...
  useEffect(() => {
    ...
    async function load(lat: number, lon: number, city: string) {
      const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code,relative_humidity_2m&daily=temperature_2m_max,temperature_2m_min&forecast_days=1&timezone=auto`;
      ...
      const next: WeatherNow = {
        ...
        high: json.daily?.temperature_2m_max?.[0] == null ? null : Math.round(json.daily.temperature_2m_max[0]),
        low: json.daily?.temperature_2m_min?.[0] == null ? null : Math.round(json.daily.temperature_2m_min[0]),
